...
import br.com.gatica.codetickets.mapper.ImportacaoMapper;
...
public class ImportacaoJobConfiguration {
    ...
    @Bean
    public Step passoInicial(ItemReader<Importacao> itemReader, ItemWriter<Importacao> itemWriter, JobRepository jobRepository) {
        return new StepBuilder("passo-inicial", jobRepository)
                .<Importacao, Importacao>chunk(100, transactionsManager)
                .reader(itemReader)
                .processor(processor())
                .writer(itemWriter)
                .build();
    ...
    @Bean
    public ItemReader<Importacao> itemReader() {
    ...
    @Bean
    public ItemWriter<Importacao> itemWriter(DataSource dataSource) {
    ...
    public Step moverArquivosStep(JobRepository jobRepository) {
        return new StepBuilder("mover-arquivo", jobRepository)
                .tasklet(moverArquivosTasklet(), transactionsManager)
                .build();
    }
